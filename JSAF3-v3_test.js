    ...
    document.getElementById('getenablerAP').onclick = function () {               // сохранение в буфере ссылки для активации АП
		...
		function getJiraTask() {
			...
			//   rezissuetable = JSON.parse(rezissuetable)
			   ...
			   let arraytmpl=[];
			   for (let i=0; i<10; i++) {
				   issues += '<a href="https://jira.skyeng.tech/browse/' + rezissuetable.issueTable.issueKeys[i] + '" onclick="" style="color: #FFE4C4">' + rezissuetable.issueTable.table.match(/browse.*?(\D+\d+".\D+)<\/a>/gm)[i] + '</a></br>'			   
			   }
