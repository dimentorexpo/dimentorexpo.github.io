...
function prepTp() { //функция подготовки расширения ТП
	...
	let create = (info) => {
		...
		let gfgScript = ["https://dimentorexpo.github.io/jquery-3.6.0.js", // подключаем модуль обработки JQuery
						 ...
						 "https://dimentorexpo.github.io/DateTimePicker/jquery.dateandtime.js", // подключаем библиотеку для работы с датами и временем
...
function checkelementtype(a) { // проверка на какой элемент нажали
    ...
    if (elem.nodeName != 'BUTTON' && elem.nodeName != 'INPUT' && elem.nodeName != 'DIV' && elem.nodeName != 'TEXTAREA' && elem.nodeName != 'SELECT' && elem.className != "checkbox-audio-switch") {
        return true;
    }
    return false;
}
