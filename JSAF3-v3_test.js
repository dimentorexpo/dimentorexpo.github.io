...
document.getElementById('getJiraTasks').onclick = function () {
    ...
    document.getElementById('firstpage').addEventListener('click', function () {
        ...
        document.getElementById('issuetable').innerHTML = issues;
         ...
         let refreshissuesarr3 = document.querySelectorAll('.refreshissues3');
            for (let f = 0; f < refreshissuesarr3.length; f++) {
                refreshissuesarr3[f].onclick = function () {

                    document.getElementById('responseTextarea1').value = '{}'
                    document.getElementById('responseTextarea2').value = "https://jira.skyeng.tech/secure/AjaxIssueEditAction!default.jspa?decorator=none&issueId=" + rezissuetable.issueTable.issueIds[f]
                    document.getElementById('responseTextarea3').value = 'reportscount3'
                    document.getElementById('sendResponse').click()
                    ...
                    setTimeout(function () {
                        ...
                        let repcount3 = document.getElementById('responseTextarea1').getAttribute('reportscount3')
                        jira_token3 = repcount3.match(/"atl_token":"(.*lin)/)[1]
						...
						"body": "customfield_15410=${increasedcount3}&issueId=${rezissuetable.issueTable.issueIds[f]}&atl_token=${jira_token3}&singleFieldEdit=true&fieldsToForcePresent=customfield_15410",
						  "method": "POST",
						  "mode": "cors",
						  "credentials": "include"
							}`
	...
	await fetch("https://skyeng.autofaq.ai/api/conversations/history", {
        ...
        while (true) {
            ...
            for (let i = 0; i < test.items.length; i++) {
                ...
                if (flagTopic == 1)
                    stringChatsWithoutTopic2 += '<span style="color: #00FA9A">&#5129;</span>' + " " + '<a href="https://hdi.skyeng.ru/autofaq/conversation/-11/' + test.items[i].conversationId + '" onclick="" style="color:#008B8B;">' + test.items[i].conversationId + '</a></br>'
            }
        ...
        for (let i = 0; i < test.items.length; i++) {
            ...
            await fetch('https://skyeng.autofaq.ai/api/conversations/' + test.items[i].conversationId)
                .then(response => response.json()).then(data => {
                    ...
                    if (flagComment == 1)
                        stringChatsWithComment += '<span style="color: #00FA9A">&#5129;</span>' + " " + '<a href="https://hdi.skyeng.ru/autofaq/conversation/-11/' + data.id + '" onclick="" style="color:#008B8B;" class="chatids">' + data.id + '</a>' + '<span class = "chatswithcomments" style="margin-left: 10px; cursor: pointer">👁‍🗨</span>' +'</br>'
            ...
            for (let i = 0; i < test.items.length; i++) {
                ...
                if (flagCsat1 == 1)
                    if (test.items[i].stats.rate != undefined)
						...
						stringChatsWithLowCsat += '<span style="color: #00FA9A">&#5129;</span>' + " " + '<a href="https://hdi.skyeng.ru/autofaq/conversation/-11/' + test.items[i].conversationId + '" onclick="" style="color:#008B8B;" class = "csatchatids">' + test.items[i].conversationId + '</a>' + '<span class = "lowcsatschats" style="margin-left: 10px; cursor: pointer">👁‍🗨</span>' + '</br>'
