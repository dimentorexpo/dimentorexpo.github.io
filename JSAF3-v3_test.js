	...
	for(i = 0; document.getElementsByClassName('expert-user_details-list')[1].childNodes[i] != undefined; i++) {
	 ...
	 document.getElementById('sendResponse').click()
	  ...
	  function getUserAgentInfo() {
		...
		if (getMyResult.match(/\d+/) > 0 && getMyResult.match(/.{13}система.{114}/).split('<br/>') == "Тип клиентского приложения: Веб-браузер") {	
			document.getElementById("nextStudentUserAgent").innerHTML = getMyResult.match(/.{13}система.{114}/)[0];
			} else if (getMyResult.match(/\d+/) > 0 && getMyResult.match(/.{13}система.{114}/).split('<br/>') == "Тип клиентского приложения: Мобильное приложение") {
	...
	for(i = 0; document.getElementsByClassName('expert-user_details-list')[1].childNodes[i] != undefined; i++) {
	 ...
	 document.getElementById('sendResponse').click()
	  ...
	  function getUserAgentInfo2() {
		...
		if (getMyResult.match(/\d+/) > 0 && getMyResult.match(/.{13}система.{114}/).split('<br/>') == "Тип клиентского приложения: Веб-браузер") {	
			document.getElementById("nextTeacherUserAgent").innerHTML = getMyResult.match(/.{13}система.{114}/)[0];
			} else if (getMyResult.match(/\d+/) > 0 && getMyResult.match(/.{13}система.{114}/).split('<br/>')[1] == "Тип клиентского приложения: Мобильное приложение") {
