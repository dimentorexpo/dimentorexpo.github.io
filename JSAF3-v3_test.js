...
wintChatHis.style = 'min-height: 25px; min-width: 65px; height:100vh; background: rgb(70, 68, 81); top: 0px; left: 80.6%; font-size: 14px; z-index: 20; position: fixed; border: 1px solid rgb(56, 56, 56); color: black;';
                ...
                for (let i = 0; i < data.items.length; i++) {
                    ...
                    if (data.items[i].stats.usedStatuses == "AssignedToOperator")
						...
						if (data.items[i].channelUser.payload.userFullName == undefined)							
							foundarr += '<span class="chatlist" style="cursor:pointer;">' + day + '.' + month + '.' + year + ' ' + tshrs + ':' + tsmin + ' ' + '<span style ="color:#00BFFF; font-weight:700">' + data.items[i].channelUser.payload.userType + '</span>' + ' ' + data.items[i].channelUser.userFullName + '<span style="color:YellowGreen">' + ' Оценка: ' + '</span>' + marksarr + actstatus + '</span>' + '<br>'								                           
						else
                    ...
                    if (objSel[i].selected == true) {
                        ...
                        for (let i = 0; i < operchatsdata.items.length; i++) {
							...
							if (operchatsdata.items[i].channelUser.payload.userFullName == undefined)
								foundarr += '<span class="chatlist" style="cursor:pointer;">' + day + '.' + month + '.' + year + ' ' + tshrs + ':' + tsmin + ' ' + '<span style ="color:#00BFFF; font-weight:700">' + operchatsdata.items[i].channelUser.payload.userType + '</span>' + ' ' + operchatsdata.items[i].channelUser.userFullName + '</span>' + '<br>'                            
							else
