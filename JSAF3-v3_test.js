			...
			<span style="cursor: -webkit-grab;">
				...
				<div style="margin: 5px; width: 550px" id="links_butd">	
					...
					<button title="отображает актуальную версию iOS приложения" id="curVeriOS" style="float: right; margin-right: 10px;">iOS: 9.63</button>
			  	    <button title="Отображает актуальную версию Android приложения" id="curVerAndroid" style="float: right; margin-right: 5px;">Аndroid: 9.59</button>
...
function move_again_AF() {
	...
	document.getElementById('butMarks').onclick = function () {
		...
		for (let i=0; i<datamarks.items.length;i++) {
			...
			document.getElementById('markstable').innerHTML = 'Оценка 1 🤬: ' + count[1] + ' | ' + ((count[1]/markscount)*100).toFixed(1)  + '<br>' + 'Оценка 2 🤢: ' + count[2] + ' | ' + (((count[2]/markscount)*100).toFixed(1) + '<br>' + 'Оценка 3 😐: ' + count[3] + ' | ' + (((count[3]/markscount)*100).toFixed(1) + '<br>'+ 'Оценка 4 🥴: ' + count[4] + ' | ' + ((count[4]/markscount)*100).toFixed(1) + '<br>' + 'Оценка 5 😊: ' + count[5] + ' | ' + ((count[5]/markscount)*100).toFixed(1)*100 + '<br>' + 'Всего оценок: ' + markscount + '<br>' + 'Обращений с начала года: ' + datamarks.total ;			
