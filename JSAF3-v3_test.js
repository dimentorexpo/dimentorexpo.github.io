    ...
    `<div style="display: flex; width: 524px;">
        <span style="width: 524px">
                <span style="cursor: -webkit-grab;">
						...
						<span id="firstpage"> <a style="float: left; margin-right: 5px; margin-top: 10px; color: #00FFFF; text-align: center; font-size: 16px; cursor: pointer;">1</a>';</span>
						<span id="secondpage"> <a style="float: left; margin-right: 5px; margin-top: 10px; color: #00FFFF; text-align: center; font-size: 16px; cursor: pointer;">2</a>';</span>
						<div>
                </span>
        </span>
</div>`;
    ...
    document.getElementById('getenablerAP').onclick = function () {               // сохранение в буфере ссылки для активации АП
		...
		function getJiraTask() {
			...
			//   rezissuetable = JSON.parse(rezissuetable)
			   ...
			   let arraytmpl=[];
			   for (let i=0; i<10; i++) {
				   issues += '<a href="https://jira.skyeng.tech/browse/' + rezissuetable.issueTable.issueKeys[i] + '" onclick="" style="color: #FFE4C4">' + rezissuetable.issueTable.table.match(/\w+\W+\d+">(\D+)<\/a>/gm)[i] + '</a></br>'			   
			   }
