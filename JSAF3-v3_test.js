						...
						for (let v=0; v<addtofarr.length; v++) {
							addtofarr[v].onclick = function() {
								...
								for (let x=0; x<tagsarray.length; x++) {
										if (x == v) {
											favissues.push('<span style="color: #00FA9A">&#5129;</span>' + `<a name="favbugs" href="${tagsarray[x].href}" target="_blank" style="color:bisque;">` + tagsarray[x].innerHTML + '</a>' + `<span name="favissuemassive">${massivissueids[x].innerText} style="display:none">` + '</span>' + '<span name="addtonotesbug" style="cursor:pointer;" title="Добавить в комментарий в чат и в ссылку на Jira">💬</span>' + '<span name="removefromfavourites" style="cursor:pointer;" title="Удалить задачу из Избранного">❌</span>' + '<span name = "increasecount" style="color:#ADFF2F; margin-left: 5px; cursor: pointer">&#69717;&#120783;</span>' + '<br>')
											localStorage.setItem('bugsarray', JSON.stringify(favissues))
										}
								}
							}
