...
let stid;
	...
	async function checkServiceAndUserInfo() {
		if (flagusertype =="teacher") {
		 ...
		 console.log('checkServiceAndUserInfo started');
	  const responseTextarea1 = document.getElementById('responseTextarea1');
			...
			srvparsed.data.forEach((service, i) => {
				...
				if(service.student.general.id == stid) {
					if (service.stage === "after_trial" || service.stage === "before_call") {
						servinfo += '<div style="text-align:center; background:#8f8a83; font-weight:700;border: 1px solid black;">Этап ВУ |' + ' 💰 Баланс: ' + service.balance + '</div>' + '<div style="background: #d59f34; color:#000000;  margin-left: 5px; border: 1px solid bisque;">' + [i + 1] + ") " + '<span>🆔 Услуги: </span>' + service.id + '<span class = "copyserviceid">💾</span>' + '<br>' + '💡:' + service.serviceTypeKey + '</div>'
						arrservice += service.id + ", ";
					}

					if (service.stage === "regular_lessons") {
						...
						servinfo += '<div style="text-align:center; background:#8f8a83; font-weight:700;border: 1px solid black;">Регулярные занятия |' + ' 💰 Баланс: ' + service.balance + '</div>' + '<div style="background: #2b602b; color:#FFF;  margin-left: 5px; border: 1px solid bisque;">' + [i + 1] + ") " + '<span>🆔 Услуги: </span>' + service.id + '<span class = "copyserviceid">💾</span>' + '<br>' + '💡:' + service.serviceTypeKey + '<br>' + teacherInfo + '</div>';
						arrservice += service.id + ", ";
					}

					
					if (service.stage === "lost") {
						servinfo += '<div style="text-align:center; background:#8f8a83; font-weight:700;border: 1px solid black;">Потерянная услуга |' + ' 💰 Баланс: ' + service.balance + '</div>' + '<div style="background: #5a0f77; color:#c6c5c5;  margin-left: 5px; border: 1px solid bisque;">' + [i + 1] + ") " + '<span>🆔 Услуги: </span>' + service.id + '<span class = "copyserviceid">💾</span>' + '<br>' + '💡:' + service.serviceTypeKey + '</div>'
						arrservice += service.id + ", ";
					}
				}
