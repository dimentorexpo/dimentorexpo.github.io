...
document.getElementById('serviceinf').innerHTML = '';
    ...
    if (document.getElementById('AF_Createtask').style.display == 'none') {
        ...
        document.getElementById("responseTextarea1").addEventListener("DOMSubtreeModified", function () {
			...
			if (iframenewUI[0].querySelectorAll('[class^="sash-container"]').length != 1) {
				let perechenAF = SearchinAFnewUI("nextClass-statusHTML")
					if (perechenAF == "идёт урок" || perechenAF == "идет урок" || perechenAF == "идет ВУ" || perechenAF == "идёт ВУ" || perechenAF == "идёт вводный урок") {
						document.getElementById('statusuroka').innerHTML = perechenAF
		...
		let activeConvId = getActiveConvid();
        ...
        document.getElementById('createtask').onclick = function () {
			...
			let sidebarUserInfo = iframenewUI[0].querySelectorAll('ul[class*="Variables_List"]');
