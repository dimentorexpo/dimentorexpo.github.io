...
function locateiframe() {
    ...
    if (iframe) {
      ...
      const nextElement = iframeDocument.querySelectorAll("#__next");
      
      if (nextElement.length > 0) {
        iframenewUI = nextElement;
      }
    }
}
...
function newTaggg(tagName) { //функция добавления тега в чат, но надо потом искать где используется
    let chatId = ''
				...
				let newFrameIndex;
			let tes1 = document.getElementsByTagName('iframe')[0].contentDocument.children[0].children[1]
...
function getActiveConvid() {
    ...
    for (let i = 0; i < ConvArray.length; i++) {
      if (ConvArray[i].getAttribute('aria-selected') === 'true') {
        return ConvArray[i].getAttribute('data-conv-id');
      }
    }
