					...
					for (let j=0; j<srvcont.data.length;j++) {
						if(srvcont.data[j].serviceTypeKey == usersrvparsed.data[i].serviceTypeKey) {
							...
							if (usersrvparsed.data[i].incorrectnessReason == null) {
								if ( .stage == 'regular_lessons') {
									document.getElementById('serviceinf').innerHTML += `<div class="srvhhelpnomove" name="outservfield" title="${usersrvparsed.data[i].id}" style="background: rgb (25, 135, 84); color:bisque;  margin-left: 5px; border: 1px solid bisque;">` + '🆔 услуги: ' + usersrvparsed.data[i].id + ' — ' + usersrvparsed.data[i].serviceTypeKey + '<span class="srvhhelpnomove" name="movetoservid" title="По клику перенесет ID услуги в поле создания задачи" style="cursor:pointer; font-size:16px;"> ➡</span>' + '<br>' + '👨‍🎓 Student: ' + usersrvparsed.data[i].student.general.id + ' ' + usersrvparsed.data[i].student.general.name + ' ' + usersrvparsed.data[i].student.general.surname + '<br>' + '👽 Teacher:' + (usersrvparsed.data[i].teacher != null ? usersrvparsed.data[i].teacher.general.id + ' ' + usersrvparsed.data[i].teacher.general.name + ' ' + usersrvparsed.data[i].teacher.general.surname : ' —') + '</div>'
								}
							}
						}	
					}
