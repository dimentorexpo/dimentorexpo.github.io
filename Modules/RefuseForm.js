        ...
        <span style="width: 414px">
                <span style="cursor: -webkit-grab;">
                        <div style="margin: 5px; width: 410px;" id="refuse_form_header">
                            <button title="скрывает меню" id="hideMeRefuseFormv2" class="buttonHide">hide</button>
...
var listenerRefuseForm = function (e, a) { // сохранение позиции окна отказов
    wintRefuseFormNew.style.left = Number(e.clientX - myXRefuseForm) + "px";
    wintRefuseFormNew.style.top = Number(e.clientY - myYRefuseForm) + "px";
    localStorage.setItem('winTopRefuseNew', String(Number(e.clientY - myYRefuseForm)));
    localStorage.setItem('winLeftRefuseNew', String(Number(e.clientX - myXRefuseForm)));
};

wintRefuseFormNew.onmousedown = function (a) { // изменение позиции окна отказов
    if (checkelementtype(a)) {
        window.myXRefuseForm = a.layerX;
        window.myYRefuseForm = a.layerY;
