	  ...
	  for (let i = 0; i < opstats.length; i++) {
		...
		switch (operator.status) {
		  ...
		  case "Pause":
			...
			spanClass = "font-weight:700;";
			break;
		  default:
			continue;
		}
		moderresult += `<div class="${divClass}" style="${divStyle}" name="operrow" value="${operator.id}">` +
		  `<span style="color: white; font-size: 13px; ${spanClass} text-shadow: rgb(191 125 125) 1px 0px 1px, rgb(191 125 125) 0px 1px 1px, rgb(191 125 125) -1px 0px 1px, rgb(191 125 125) 0px -1px 1px; background: ${spanClass ? "FireBrick" : "green"}; width: 25px; height: 25px; padding-top:2px; text-align: center; border-radius: 50%; border: 1px solid black;">` + 
		  `${opstats[i].aCnt}` + 
		  `</span>` + 
		  `${operator.fullName}` + 
		  '</div>';
