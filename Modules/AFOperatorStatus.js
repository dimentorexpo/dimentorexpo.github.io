        ...
        for (let i = 0; i < result.onOperator.length; i++) {
			if (result.onOperator[i].operator.status != "Offline") {
				if (flagtpkc == 'ТП' && result.onOperator[i].operator != null && result.onOperator[i].operator.fullName.match(/ТП\D/)) {
					...
					opstats.push(result.onOperator[i])
					for (let j=0; result.unAssigned[j] != undefined; j++) {
						if (result.unAssigned[j].kb != '120181' && result.unAssigned[j].kb != '121300' && result.unAssigned[j].kb != '121438' && result.unAssigned[j].kb != '121386' && result.unAssigned[j].kb != '121446' && result.unAssigned[j].kb != '121832' && result.unAssigned[j].kb != '121430' && result.unAssigned[j].kb != '121879' && result.unAssigned[j].kb != '121777' && result.unAssigned[j].kb != '121447' && result.unAssigned[j].kb != '121527' && result.unAssigned[j].kb != '121775' && result.unAssigned[j].kb != '121531' && result.unAssigned[j].kb != null) {
							chatneraspcountleft = result.unAssigned[j].count
						}
					}	
				...
				if (flagtpkc == 'ТП' && result.onOperator[i].operator != null && result.onOperator[i].operator.fullName.match(/ТП\D/)) {
					...
					for (let j=0; result.unAssigned[j] != undefined; j++) {
						if (result.unAssigned[j].kb != '120181' && result.unAssigned[j].kb != '121300' && result.unAssigned[j].kb != '121438' && result.unAssigned[j].kb != '121446' && result.unAssigned[j].kb != '121832' && result.unAssigned[j].kb != '121430' && result.unAssigned[j].kb != '121879' && result.unAssigned[j].kb != '121777' && result.unAssigned[j].kb != '121447' && result.unAssigned[j].kb != '121527' && result.unAssigned[j].kb != '121531' && result.unAssigned[j].kb != '121775' && result.unAssigned[j].kb != null) {
							chatneraspcountleft = result.unAssigned[j].count
						}
					}
				} // end of if state small 	
...
const blockedChatCount = `🚧 Нераспред: ${chattpquecountleft}`;
const toggleSummaryBtn = `<div id="clicktounhidestatuses" title="По клику откроет общую информацию о том, сколько всего операторов и их количество в разных статусах" style="color:bisque; opacity:0.8; cursor:pointer; text-align:center;">${localStorage.getItem('hidesummaryflag') === '1' ? '🔽 Открыть' : '🔼 Скрыть'}</div>`;
const operatorStatuses = `<div id="opersstats" style="display: ${localStorage.getItem('hidesummaryflag') === '1' ? 'none' : 'block'}">
  <div style="background:#257947; font-weight: 700; text-align: center; border: 1px solid black;">🛠 Онлайн: ${operonlinecnt}</div>
  <div style="background: #a3bb1d; color: black; font-weight: 700; text-align: center; border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black;">⏳ Занят: ${busycnt}</div>
  <div style="background:#cf4615; font-weight: 700; text-align: center;border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black;">🍔 Перерыв: ${pausecnt}</div>
  <div style="background:#492579; font-weight: 700; text-align: center;border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black;">⚡ Всего: ${+pausecnt + busycnt + operonlinecnt}</div>
...
if (flagtpkc === 'ТП' && localStorage.getItem('hidesummaryflag') === '1') {
    ...
    peoplestatus.innerHTML = toggleSummaryBtn;
} else if (flagtpkc !== 'ТП' && localStorage.getItem('hidesummaryflag') === '0') {
    peoplestatus.innerHTML = `
        ${toggleSummaryBtn}
        ${operatorStatuses}
