    ...
    if (opstats.length != 0) {
        for (let i = 0; i < opstats.length; i++) {
            ...
            if (opstats[i].operator.status == "Online") {
                ...
                moderresult += `<div class="leftbaropers" style="opacity:0.8; color:Salmon" name="operrow" value="${opstats[i].operator.id}">` + '<span style="color: black; font-size: 13px; text-shadow: rgb(191 125 125) 1px 0px 1px, rgb(191 125 125) 0px 1px 1px, rgb(191 125 125) -1px 0px 1px, rgb(191 125 125) 0px -1px 1px; background: FireBrick; width: 25px; height: 25px; padding-top:2px; text-align: center; border-radius: 50%; border: 1px solid black;">' + opstats[i].aCnt + '</span>' + `${opstats[i].operator.fullName}` + '</div>'
                pausecnt += 1;
            }
        }
    } else moderresult = ''
