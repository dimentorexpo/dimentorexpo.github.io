		...
		if (opstats.length != 0) {
			for (let i = 0; i < opstats.length; i++) {
				...
				if (opstats[i].operator.status == "Online") {
					...
					moderresult += '<div style="display:flex; align-items:center;">' + '<span style="font-size:20px;">🟡 </span>' + '<span style="position: absolute;left: 10px; padding-top:3px; color:black; font-size:13px; text-shadow: rgb(191 125 125) 1px 0px 1px, rgb(191 125 125) 0px 1px 1px, rgb(191 125 125) -1px 0px 1px, rgb(f f f) 0px -1px 1px;">' + opstats[i].aCnt + '</span>' +  `${opstats[i].operator.fullName}` + '</div>'
